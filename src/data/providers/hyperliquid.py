...
class HyperliquidProvider(MarketDataProvider):
    ...
    def stream(self, *, since: datetime | None = None) -> Iterable[MarketSnapshot]:
        ...
        raise NotImplementedError("Live Hyperliquid streaming will be implemented with websockets")

    def latest(self) -> MarketSnapshot:
        if self._delegate is not None:
            return self._delegate.latest()
        raise NotImplementedError("Live Hyperliquid REST fetching not implemented yet")
