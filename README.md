...
- **Crypto agent loop (dry-run):** `poetry run python src/cli/crypto_runner.py --symbol BTCUSDT --limit 100`
  ...
  - Tune risk/execution on the fly (e.g. `--max-drawdown-bps 5000 --max-position-usd 2000 --max-order-usd 500`).
...
```bash
poetry run python scripts/cache_snapshot.py --symbol BTCUSDT --rows 300
```
...
Format with `poetry run black`, `poetry run isort`; lint via `poetry run flake8`. Tests live under `tests/` with fixtures in `tests/fixtures/`; run `poetry run pytest` (crypto specs sit under `tests/crypto/`). Keep functions minimal and explicit—avoid `try`/`except` unless propagating a required fallback.
